...
func NewQuoteRepository(db *gorm.DB) *quoteRepository {
	return &quoteRepository{
		DB:     db,
		Mapper: *NewQuoteMapper(),
	}
}

func (r *quoteRepository) GetAll(limit int, offset int) ([]*domain.Quote, error) {
	db := r.DB
...
func (r *quoteRepository) Like(quoteID int, userID int) (*domain.Quote, error) {
	db := r.DB
...
func (r *quoteRepository) Unlike(quoteID int, userID int) (*domain.Quote, error) {
	db := r.DB
...
func (r *quoteRepository) GetByAuthorName(name string, limit int, offset int) ([]*domain.Quote, error) {
	db := r.DB
	var quoteModel []model.Quote
	result := db.Preload("Author").Preload("Categories").Preload("UserWhoLiked").
...
func (r *quoteRepository) GetByAuthorID(ID int, limit int, offset int) ([]*domain.Quote, error) {
	db := r.DB
	var quoteModel []model.Quote
	result := db.Preload("Author").Preload("Categories").Preload("UserWhoLiked").
		Joins("Author").Offset(offset).Limit(limit).
...
func (r *quoteRepository) GetByCategoryName(name string, limit int, offset int) ([]*domain.Quote, error) {
	db := r.DB
	var quoteModel []model.Quote
	result := db.Preload("Author").Preload("Categories").Preload("UserWhoLiked").
...
func (r *quoteRepository) GetByCategoryID(ID int, limit int, offset int) ([]*domain.Quote, error) {
	db := r.DB
	var quoteModel []model.Quote
	result := db.Preload("Author").Preload("Categories").Preload("UserWhoLiked").
		Joins("JOIN quote_categories ON quotes.id = quote_categories.quote_id").
...
// Helper function added to the domain.Quote to extract category IDs.
func (r *quoteRepository) Update(ID int, quote *domain.Quote) (*domain.Quote, error) {
	db := r.DB
